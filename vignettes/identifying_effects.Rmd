...
Because each treatment corresponds to a different number of cells, we will use a bootstrap approach to identify indices that are significantly different between conditions. we will bootstap `r ncells` cells from the selected bins, then check which bin contains at least `r flim` times more cells in the reference compared to other treatments. This operation will be repeated `r N` times, and any bin that is consitently different in `r p*N` bins will be considered significant.
...
Is there a difference in profile between the significant and non-significant cells?
...
fullDf <- as.data.frame(cbind(fullAnnots,fullMat,'hilbert'=hc))
fullDf <- tbl_df(fullDf)

ggplot(fullDf %>% 
         filter(Cells %in% rownames(sig.cells) & Treatment %in% c('reference','BCR/FcR-XL')) %>%
         gather(channel,value,match(colnames(fullMat),colnames(fullDf))) %>%
         mutate(isSignificant=hilbert %in% names(which(sig.res[,'BCR/FcR-XL'])) &
                  Treatment!='reference') %>%
         group_by(Treatment,Cells,isSignificant,channel) %>% 
         do(do.fan(.$value)),
       aes(x=channel,fill=percent,group=id))+
  geom_ribbon(aes(ymin=ymin,ymax=ymax))+
  facet_grid(Cells*isSignificant~Treatment)+
  scale_fill_fan()+
